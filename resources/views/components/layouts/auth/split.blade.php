@php
    $preTitle = str_contains( $_SERVER['REQUEST_URI'],'register') ? "Registro" : "Conexión";
@endphp
    <!DOCTYPE html>
    ...
    <div class="bg-muted relative hidden h-full flex-col p-10 text-white lg:flex dark:border-r dark:border-neutral-800">
        ...
        <a href="{{ route('home') }}" class="relative z-20 flex items-center text-lg font-medium" wire:navigate>
                    <span class="flex h-10 w-10 items-center justify-center rounded-md">
                        <x-app-logo-icon class="mr-2 h-48 fill-current text-white"/>
                    </span>
            {!!  "$preTitle <br>". config('app.name', 'Laravel') !!}
        </a>
        @php
        ...
        <div class="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[350px]">
            <a href="{{ route('home') }}" class="z-20 flex flex-col items-center gap-2 font-medium lg:hidden"
               wire:navigate>
                ...
                <span class="sr-only">{{"$preTitle ". config('app.name', 'Laravel') }}</span>
            </a>
            {{ $slot }}
        </div>
    </div>
